...
type TerminalAction struct {
	Input   string `json:"input" jsonschema:"required" jsonschema_description:"Command to be run in the docker container terminal. Prefer non-interactive one-shot commands that finish by themselves. Avoid interactive shells and use explicit timeouts or bounded command forms when possible."`
	...
	Detach  Bool   `json:"detach" jsonschema:"required,type=boolean" jsonschema_description:"Set to true only for intentionally INTERACTIVE or LONG-RUNNING commands: shells, listeners, servers, or monitors. For verification work, prefer detach=false with one-shot commands that complete and return evidence. Do not use detach=true for service clients unless the task explicitly requires an interactive session"`
...
type TodoItem struct {
	...
	EvidenceRequirements []string `json:"evidence_requirements,omitempty" jsonschema_description:"Evidence artifacts or observations required to prove completion"`
	Status               string   `json:"status,omitempty" jsonschema_description:"Todo execution status; default is pending"`
}
