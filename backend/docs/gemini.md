...
The Google AI provider enables PentAgentX to use Google's Gemini language models through the Generative AI API. This provider supports advanced features like function calling, streaming responses, and competitive pricing.
...
docker run --rm \
  ...
  vxcontrol/pentagi /opt/pentagentx/bin/ctester -type gemini
```
...
docker run --rm \
  -v $(pwd)/.env:/opt/pentagentx/.env \
  vxcontrol/pentagi /opt/pentagentx/bin/ctester \
  -config /opt/pentagentx/conf/gemini.provider.yml

# Using custom configuration
docker run --rm \
  -v $(pwd)/.env:/opt/pentagentx/.env \
  -v $(pwd)/gemini-custom.yml:/opt/pentagentx/gemini-custom.yml \
  vxcontrol/pentagi /opt/pentagentx/bin/ctester \
  -type gemini \
  -config /opt/pentagentx/gemini-custom.yml
```
...
### Cost Management
- Monitor token usage through PentAgentX's cost tracking
...
- [PentAgentX Documentation](https://docs.pentagentx.com)
