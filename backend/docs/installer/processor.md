...
graph TB
    ...
    subgraph "External Systems"
        ...
        UpdateServer[Update Server<br/>pentagentx.com]
...
- **ProductStack**: Logical grouping of services that can be managed as a unit (pentagentx, langfuse, observability, worker, installer, all)
...
const (
    StackPentAgentX        ProductStack = "pentagentx"        // Main stack (docker-compose.yml)
...
- **Start(stack)**:
  - pentagentx/langfuse/observability: `docker compose ... up -d` (honors embedded mode for non-destructive ops)
  - all: sequential start in order observability → langfuse → pentagentx
  - worker/installer: not applicable

- **Stop(stack)**:
  - pentagentx/langfuse/observability: `docker compose ... stop`
  - all: sequential stop in reverse order (pentagentx → langfuse → observability)
  - worker/installer: not applicable
...
- **Download(stack)**:
  - pentagentx/langfuse/observability: `docker compose pull`
...
- **Install(stack)**:
  - pentagentx: extract compose file and example provider config
...
- **Update(stack)**:
  - pentagentx/langfuse/observability: download → `docker compose up -d`
...
### Removal Operations
- **Remove(stack)**:
  - pentagentx/langfuse/observability: `docker compose down` (keep volumes/images)
  - worker: remove images via Docker API and related containers
  - installer: remove flow stubbed
  - all: remove all stacks

- **Purge(stack)**:
  - pentagentx/langfuse/observability: `down --rmi all -v` for strict purge; standard purge `down -v` is also available
...
- **ApplyChanges()**:
  ...
  - phase 3: pentagentx (always embedded; ensure/verify → update)
...
- **compose.go**:
  - `installPentAgentX()`, `installLangfuse()`, `installObservability()` - extract compose files with environment patching
...
- **update.go**:
  - `checkUpdates()` - communicate with pentagentx.com update server
...
- **Dependency Ordering**: PentAgentX must start before Langfuse/Observability for network creation
...
- HTTP client for installer updates from pentagentx.com
